...
class FullNetwork(nn.Module):
    ...
    def __init__(self, vp_value_count, output_shape, name='Full Network'):
        ...
        self.gen = Generator(in_channels=self.feature_channels + self.feature_channels, out_frames=self.out_frames)
    ...
    def forward(self, vp_diff, vid1, img2):
        ...
        rep_v1 = self.action_pipeline(vid1)  # bsz,256,4,14,14

        # appearance encoding + video features
        gen_input2 = torch.cat([exp_app_v2, rep_v1], dim=1)
        ...
        return gen_v2, rep_v1

    def appearance_pipeline(self, img2):
        app_v2 = self.vgg(img2)  # bsz,32,14,14

        exp_app_v2 = self.expand_app_enc(app=app_v2, frames=self.feature_frames)  # bsz,32,8/16,14,14

        return exp_app_v2

    def action_pipeline(self, vid1):
        rep_v1 = self.i3d(vid1)  # bsz,32,4,14,14
