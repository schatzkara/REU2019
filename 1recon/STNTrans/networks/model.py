...
class FullNetwork(nn.Module):
    ...
    def action_pipeline(self, vp_diff, vid):
        ...
        for i in range(len(reps)):
            ...
            stn_input = self.prepare_stn_input(r, kp, means)
            stn_input = torch.cat([vp_1_to_2, stn_input], dim=1)  # dim=channels
            est_rep = self.STN(stn_input)
            est_rep = self.roi_pooling_helper(est_rep, kp)
            est_reps.append(self.invert_stn_input_prep(est_rep, means))

        # est_reps, est_kp = self.kpp(est_reps)  # bsz,32,16,56,56
        print(est_reps.size())
        return est_reps, reps[-1]

    def prepare_stn_input(self, r, kp, means):
        # roi pool
        bsz, channels, frames, height, width = r.size()
        ...
        mean_y, mean_x = means[:, :, 0:1], means[:, :, 1:2]  # bsz, nkp, frames, h/w=1
        print(means)

        rois = torch.zeros(bsz * nkp, 5)
        for i in range(bsz * nkp):
            rois[i] = [i, mean_x[i] - 2, mean_y[i] - 2, mean_x[i] + 2, mean_y[i] + 2]

        x = roi_pool(image=r, rois=rois)
