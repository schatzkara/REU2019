...
from networks.ofp import OFPredictor
...
class FullNetwork(nn.Module):
    ...
    def __init__(self, vp_value_count, stdev, output_shape, use_est_vp=False, name='Full Network'):
        ...
        self.vpp = VPPredictor(in_channels=self.app_feat + self.rep_feat)
    ...
    def appearance_pipeline(self, apps, est_reps, est_kp):
        ...
        for i in range(len(apps)):
            ...
            est_of = self.ofp(r, kp)
            cell_input, r, kp = est_of.permute(0, 2, 1, 3, 4), r.permute(0, 2, 1, 3, 4), kp.permute(0, 2, 1, 3, 4)
            # cell_input = torch.cat([r, kp], dim=1).permute(0, 2, 1, 3, 4)  # dim=channels
            output, last_state = self.gru(input_tensor=cell_input, mf=r, kp=kp,
                                          hidden_state=[a])
