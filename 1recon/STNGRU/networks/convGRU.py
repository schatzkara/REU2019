...
class ConvGRUCell(nn.Module):
    ...
    def forward(self, input_tensor, h_cur, rep, kp, rois):
        """
        :param self:
        ...
        # cropped_app, cropped_rep = self.attentive_crop(h_cur, rois), self.attentive_crop(rep, rois)
        # cnm = self.STN(cropped_rep, cropped_app)
        cnm = self.tranform_features(h_cur, rep, rois)

        h_next = (1 - update_gate) * h_cur + update_gate * cnm
        return h_next

    def transform_features(self, app, rep, rois):
        bsz, channels, height, width = app.size()

        cropped_height = height // 4
        cropped_width = width // 4
        app_trans = torch.zeros(bsz, channels, height, width)

        mask = nn.Softmax(dim=1)((rois > 1).float())

        mean_y, mean_x = rois[:, :, 0:1], rois[:, :, 1:2]
        for i in range(rois.size()[1]):
            bottom, top = mean_y - (cropped_height // 2), mean_y + (cropped_height // 2) + 1
            left, right = mean_x - (cropped_width // 2), mean_x + (cropped_width // 2) + 1

            cropped_app, cropped_rep = app[:, :, bottom:top, left:right], rep[:, :, bottom:top, left:right]
            transformed = self.STN(cropped_rep, cropped_app)

            pad_bottom, pad_top = bottom, height - top
            pad_left, pad_right = left, (width - right)
            pad = nn.ZeroPad2d((pad_left, pad_right, pad_top, pad_bottom))

            app_trans = app_trans + (pad(transformed[:, :, bottom:top, left:right]) * mask)
...
class ConvGRU(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        """
        r, kp, rois = input_tensor
        input_tensor = torch.cat([r, kp], dim=1).permute(0, 2, 1, 3, 4)  # dim=channels
        r, kp, rois = r.permute(0, 2, 1, 3, 4), kp.permute(0, 2, 1, 3, 4), rois.permute(0, 2, 1, 3)
        ...
        for layer_idx in range(self.num_layers):
            ...
            for t in range(seq_len):
                ...
                # then compute the next hidden and cell state through ConvGRUCell forward function
