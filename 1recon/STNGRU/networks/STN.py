...
class STN(nn.Module):
    ...
    def stn(self, trans_from, trans_to):
        xs = self.localization(trans_from)
        xs = xs.view(-1, 10 * self.out_shape[0] * self.out_shape[1])
        theta = self.fc_loc(xs)
        theta = theta.view(-1, 2, 3)

        grid = F.affine_grid(theta, trans_to.size())
        x = F.grid_sample(trans_to, grid)

        return x

    def forward(self, trans_from, trans_to):
        # transform the input
        x = self.stn(trans_from, trans_to)
