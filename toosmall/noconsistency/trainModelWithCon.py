...
def ntu_config():
    ...
    if not os.path.exists('./weights'):
        os.mkdir('./weights')
    weight_file = './weights/netwcon_ntu_{}_{}_{}_{}_{}_{}.pt'.format(BATCH_SIZE, FRAMES, SKIP_LEN,
                                                                      PRECROP, NUM_EPOCHS, LR)
    pretrained_weights_file = './weights/netnocon_ntu_{}_{}_{}_{}_{}_{}.pt'.format(BATCH_SIZE, FRAMES, SKIP_LEN,
                                                                                   PRECROP, NUM_EPOCHS, LR)

    return data_root_dir, train_split, test_split, param_file, weight_file, pretrained_weights_file
...
def panoptic_config():
    ...
    if not os.path.exists('./weights'):
        os.mkdir('./weights')
    weight_file = './weights/netwcon_pan_{}_{}_{}_{}_{}_{}.pt'.format(BATCH_SIZE, FRAMES, SKIP_LEN,
                                                                      PRECROP, NUM_EPOCHS, LR)
    pretrained_weights_file = './weights/netnocon_pan_{}_{}_{}_{}_{}_{}.pt'.format(BATCH_SIZE, FRAMES, SKIP_LEN,
                                                                                   PRECROP, NUM_EPOCHS, LR)

    return data_root_dir, train_split, test_split, close_cams_file, weight_file, pretrained_weights_file
...
def training_loop(epoch):
    ...
    for batch_idx, (vp_diff, vid1, vid2) in enumerate(trainloader):
        ...
        loss = CON_LOSS_W * con_loss + recon1_loss + recon2_loss
        loss.backward()
        optimizer.step()
    ...
    print('Training Epoch {}/{} Loss:{} con:{} recon1:{} recon2:{}'.format(
        epoch + 1,
        NUM_EPOCHS,
        "{0:.5f}".format((running_total_loss / len(trainloader))),
        "{0:.5f}".format((running_con_loss / len(trainloader))),
...
def testing_loop(epoch):
    ...
    print('Validation Epoch {}/{} Loss:{} con:{} recon1:{} recon2:{}'.format(
        epoch + 1,
        NUM_EPOCHS,
...
def print_params():
    ...
    print('started using con loss after 50 epochs')
...
if __name__ == '__main__':
    ...
    if DATASET.lower() == 'ntu':
        data_root_dir, train_split, test_split, param_file, weight_file, pretrained_weights_file = ntu_config()
        ...
        data_root_dir, train_split, test_split, close_cams_file, weight_file, pretrained_weights_file = panoptic_config()
