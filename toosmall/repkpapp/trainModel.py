...
def ntu_config():
    ...
    if not os.path.exists('./weights'):
        os.mkdir('./weights')
    weight_file = './weights/net3in_ntu_{}_{}_{}_{}_{}_{}.pt'.format(BATCH_SIZE, FRAMES, SKIP_LEN,
...
def panoptic_config():
    ...
    if not os.path.exists('./weights'):
        os.mkdir('./weights')
    weight_file = './weights/net3in_pan_{}_{}_{}_{}_{}_{}.pt'.format(BATCH_SIZE, FRAMES, SKIP_LEN,
                                                                    PRECROP, NUM_EPOCHS, LR)
...
def training_loop(epoch):
    ...
    for batch_idx, (vp_diff, vid1, vid2) in enumerate(trainloader):
        ...
        loss = CON_LOSS_W * (con1_loss + con2_loss + con3_loss + con4_loss) + recon1_loss + recon2_loss
        loss.backward()
        optimizer.step()
        ...
        if (batch_idx + 1) % 10 == 0:
            print('\tBatch {}/{} Loss:{} con1:{} con2:{} con3:{} con4:{} recon1:{} recon2:{}'.format(
                batch_idx + 1,
                len(trainloader),
                "{0:.5f}".format(loss),
    ...
    print('Training Epoch {}/{} Loss:{} con1:{} con2:{} con3:{} con4:{} recon1:{} recon2:{}'.format(
        epoch + 1,
        NUM_EPOCHS,
        ...
        "{0:.5f}".format((running_con3_loss / len(trainloader))),
        "{0:.5f}".format((running_con4_loss / len(trainloader))),
...
def testing_loop(epoch):
    ...
    print('Validation Epoch {}/{} Loss:{} con1:{} con2:{} con3:{} con4:{} recon1:{} recon2:{}'.format(
        epoch + 1,
        NUM_EPOCHS,
        ...
        "{0:.5f}".format((running_con3_loss / len(testloader))),
