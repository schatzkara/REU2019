...
class FullNetwork(nn.Module):
    ...
    def forward(self, vp_diff, vid1, img2):
        ...
        :return: The output video from the second viewpoint.
                 Shape of output video is: (bsz, 3, 8/16, 112, 112) for this application.
        """
        app_v2 = self.vgg(img2)  # bsz,32,28,28

        app_v2 = self.expand_app_encoding(app=app_v2)  # bsz,32,8/16,28,28

        rep_v1 = self.i3d(vid1)  # bsz,256,4,7,7

        # these are the keypoint feature maps that get returned
        kp_v1 = self.deconv(rep_v1)  # bsz,32,8/16,28,28
        ...
        gen_input2 = torch.cat([app_v2, kp_v1], dim=1)  # dim=channels

        # these are the videos that get returned
        output_v2 = self.gen(gen_input2)  # bsz,3,8/16,112,112
