...
from testNetwork import FullNetwork
from NTUDataLoader import NTUDataset
from PanopticDataLoader import PanopticDataset
from outputVideoConversion import convert_to_vid
...
def panoptic_config():
    ...
    if not os.path.exists('./weights'):
        os.mkdir('./weights')
    weights_path = '/home/yogesh/kara/REU2019/phase0.5/weights/net2_pan2_20_16_2_False_1000_0.0001.pt'
    output_video_dir = './videos/just_testing'
...
def test():
    ...
    for batch_idx, (vp_diff, vid1, vid2) in enumerate(testloader):
        ...
        with torch.no_grad():
            output_vid2 = model(vp_diff=vp_diff, vid1=vid1, img2=img2)
        ...
        if (batch_idx + 1) % 10 == 0:
            print('\tBatch {}/{} Loss:{} recon2:{}'.format(batch_idx + 1, len(testloader), "{0:.5f}".format(loss),
                                                           "{0:.5f}".format(recon2_loss)))

    print('Testing Complete Loss:{} recon2:{}'.format(
