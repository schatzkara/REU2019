...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        x = f.interpolate(x, size=(self.out_frames, 56, 56), mode='bilinear')
        x = self.conv3d_2a(x)
        x = self.conv3d_2b(x)

        x = f.interpolate(x, size=(self.out_frames, 112, 112), mode='bilinear')
        x = self.conv3d_3a(x)
        x = self.conv3d_3b(x)
...
#
#     if print_summary:
#         summary(gen, input_size=(1, 16, 28, 28))
