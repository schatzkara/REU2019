# makes a file with each camera and the cameras on the same panel as it for each sample
...
def get_same_panel_cams(vga_list):
    same_panel_cams = {}
    for vga1 in vga_list:
        cams = []
        type1, panel1, node1 = vga1.split('_')
        for vga2 in vga_list:
            if vga1 == vga2:
                ...
                if panel1 == panel2:
                    cams.append(vga2)
        if len(cams) > 0:
            same_panel_cams[vga1] = cams

    return same_panel_cams
...
if __name__ == '__main__':
    ...
    with open(output_file, 'w') as f:
        for sample in sample_list:
            ...
            same_panel_cams = get_same_panel_cams(vga_list)
            print(same_panel_cams)
            for cam, close_cams in same_panel_cams.items():
