...
class NTUDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        action, sample_id, nf_v1, nf_v2 = self.process_index(index=idx)
        view1path, view2path = self.get_vid_paths(action=action, sample_id=sample_id)

        frame_count = min(nf_v1, nf_v2)
        frame_index = NTUDataset.rand_frame_index(frame_count=frame_count,
                                                  clip_len=self.clip_len, skip_len=self.skip_len)
        pixel_index = self.rand_pixel_index(height=self.height, width=self.width,
                                            desired_height=self.resize_height, desired_width=self.resize_width)
    ...
    @staticmethod
    def rand_frame_index(frame_count, clip_len, skip_len=1):
        """
    ...
    def load_frames(self, vid_path, clip_len, skip_len, frame_index, pixel_index,
                    resize_height, resize_width, channels):
        """
