...
def ntu_config():
    ...
    weights_path = '/home/yogesh/kara/REU2019/phase0.5/weights/nettrans_ntu_20_16_2_True_1000_0.0001.pt'
    output_video_dir = './videos/ntutrans_12epochs'
...
def panoptic_config():
    ...
    if not os.path.exists('./weights'):
        os.mkdir('./weights')
    weights_path = '/home/yogesh/kara/REU2019/phase0.5/weights/net_panoptic_20_16_2_False_1000_0.0001.pt'
    output_video_dir = './videos/pantrans_xepochs/'
...
def test():
    ...
    for batch_idx, (vp_diff, vid1, vid2) in enumerate(testloader):
        ...
        with torch.no_grad():
            ...
            if PRINT_KP:
                convert_to_vid(kp_v1[:,:3,:,:,:], output_video_dir, batch_idx + 1, 1, 'kp')
                convert_to_vid(kp_v2[:,:3,:,:,:], output_video_dir, batch_idx + 1, 2, 'kp')
                convert_to_vid(kp_v1_est[:,:3,:,:,:], output_video_dir, batch_idx + 1, 1, 'estkp')
                convert_to_vid(kp_v2_est[:,:3,:,:,:], output_video_dir, batch_idx + 1, 2, 'estkp')
