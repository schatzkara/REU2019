...
class FullNetwork(nn.Module):
    ...
    def __init__(self, vp_value_count, output_shape, stdev, name='Full Network'):
        ...
        self.kpp = KPPredictor(in_channels=self.feature_channels, nkp=self.nkp, stdev=stdev)

        self.exp = Expander(vp_value_count=self.vp_value_count,
                            out_frames=self.nkp_frames, out_size=self.feature_size)
        self.trans = Transformer(in_channels=self.nkp + self.vp_value_count,
                                 out_channels=self.nkp)

        self.gen = Generator(in_channels=self.feature_channels + self.nkp, out_frames=self.out_frames)
    ...
    def forward(self, vp_diff, vid1, vid2, img1, img2):
        ...
        kp_v1, kp_v2, kp_v1_est, kp_v2_est = self.action_pipeline(vp1_to_2, vp2_to_1, vid1, vid2)  # bsz,32,8/16,14,14

        # appearance encoding + video features
        gen_input1, gen_input2 = torch.cat([exp_app_v1, kp_v1_est], dim=1), torch.cat([exp_app_v2, kp_v2_est], dim=1)
        ...
        return gen_v1, gen_v2, kp_v1, kp_v2, kp_v1_est, kp_v2_est

    def appearance_pipeline(self, img1, img2):
        app_v1, app_v2 = self.vgg(img1), self.vgg(img2)  # bsz,256,14,14

        exp_app_v1, exp_app_v2 = self.expand_app_enc(app_v1, self.nkp_frames), \
                                 self.expand_app_enc(app_v2, self.nkp_frames)  # bsz,256,4,14,14
    ...
    def action_pipeline(self, vp1_to_2, vp2_to_1, vid1, vid2):
        ...
        kp_v1, kp_v2 = self.kpp(rep_v1), self.kpp(rep_v2)  # bsz,32,16,14,14

        # viewpoint change + key-points
        trans_input1, trans_input2 = torch.cat([vp2_to_1, kp_v2], dim=1), torch.cat([vp1_to_2, kp_v1], dim=1)  # dim=c

        kp_v1_est, kp_v2_est = self.trans(trans_input1), self.trans(trans_input2)  # bsz,32,16,14,14
