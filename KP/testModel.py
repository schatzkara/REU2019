...
from networks.modelWithKP import FullNetwork
...
def ntu_config():
    ...
    weights_path = './weights/netKP_ntu_{}_{}_{}_{}_{}_{}.pt'.format(BATCH_SIZE, FRAMES, SKIP_LEN,
                                                                     PRECROP, 1000, 0.0001)
    output_video_dir = './videos/ntu_netKP_'
...
def panoptic_config():
    ...
    if not os.path.exists('./weights'):
        os.mkdir('./weights')
    weights_path = './weights/netKP_pan_{}_{}_{}_{}_{}_{}.pt'.format(BATCH_SIZE, FRAMES, SKIP_LEN,
                                                                     PRECROP, 1000, 0.0001)
    output_video_dir = './videos/pan_netKP_'
...
def test():
    ...
    for batch_idx, (vp_diff, vid1, vid2) in enumerate(testloader):
        ...
        with torch.no_grad():
            gen_v1, gen_v2, kp_v1, kp_v2, kp_v1_est, kp_v2_est = model(vp_diff=vp_diff,
                                                                       vid1=vid1, vid2=vid2,
                                                                       img1=img1, img2=img2)
            ...
            convert_to_vid(tensor=kp_v1_est, output_dir=output_video_dir,
                           batch_num=batch_idx + 1, view=1, item_type='kp_est')
            convert_to_vid(tensor=kp_v2_est, output_dir=output_video_dir,
                           batch_num=batch_idx + 1, view=2, item_type='kp_est')
            ...
            loss = CON_LOSS_W * con1_loss + CON_LOSS_W * con2_loss + recon1_loss + recon2_loss
