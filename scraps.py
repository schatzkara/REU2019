...
#
#
# w = calculate_total_weighted_loss({'1':1, '2':2}, {'1':0.5})
# print(w)
...
def compute_out_size(in_size, filter_size, padding, stride):
    out_size = ((in_size - filter_size + 2 * padding) / stride) + 1
    return out_size


in_shape = (28, 28)
out_shape = (int(compute_out_size(int(compute_out_size(in_shape[0], 7, 0, 1) / 2), 5, 0, 1) / 2),
             int(compute_out_size(int(compute_out_size(in_shape[1], 7, 0, 1) / 2), 5, 0, 1) / 2))
print(out_shape)


a = torch.zeros(3, 8, 112, 112)
b = a.numpy()
