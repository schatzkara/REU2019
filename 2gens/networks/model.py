...
from networks.intermediateGen import IntermediateGen
...
class FullNetwork(nn.Module):
    ...
    def __init__(self, vp_value_count, output_shape, name='Full Network'):
        ...
        self.intergen = IntermediateGen(in_channels=32, out_frames=self.out_frames)

        self.shrink = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2))
    ...
    def forward(self, vp_diff, vid1, vid2, img1, img2):
        ...
        rep_v1, rep_v2, rep_v1_est, rep_v2_est, vid1shrunk, vid2shrunk, recon_v1, recon_v2 = self.action_pipeline(vp1,
                                                                                                                  vp2,
                                                                                                                  vid1,
                                                                                                                  vid2)
        # bsz,32,8/16,14,14
        ...
        gen_input1, gen_input2 = torch.cat([exp_app_v1, rep_v1_est], dim=1), torch.cat([exp_app_v2, rep_v2_est], dim=1)
        ...
        return gen_v1, gen_v2, vid1shrunk, vid2shrunk, recon_v1, recon_v2, rep_v1, rep_v2, rep_v1_est, rep_v2_est
    ...
    def action_pipeline(self, vp1, vp2, vid1, vid2):
        ...
        vid1shrunk, vid2shrunk = self.shrink(vid1), self.shrink(vid2)

        recon_v1, recon_v2 = self.intergen(rep_v1), self.intergen(rep_v2)

        return rep_v1, rep_v2, rep_v1_est, rep_v2_est, vid1shrunk, vid2shrunk, recon_v1, recon_v2
    ...
    def expand_app_enc(self, app):
        ...
        buffer = torch.zeros(bsz, channels, 4, height, width)
